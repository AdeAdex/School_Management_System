...
const StudentEditDetails = ({ socket }) => {
  ...
  const fetch = ()=>{
    console.log('fetching')
      // if(allmessages.length == 0) {
        axios.post('http://localhost:2000/student_account/get_student_message', {class: classIndex})
        .then((res)=>{
          console.log(res)
  ...
  const fetchGroupMessage =()=>{
    ...
    if (localStorage.myRoom=='JSS 1 Group'  && globalState.level=='JSS 1' ) {
      ...
      // index = 0
      fetch()
    } else if (localStorage.myRoom=='JSS 2 Group'  && globalState.level=='JSS 2') {
      setClassIndex(1)
      fetch()
    } else {
      // setAllmessages([])
  ...
  return (
    ...
    <div className="d-flex w-100 h-100">
      <Chat socket={socket} messages={allmessages} fetchGroupMessage={fetchGroupMessage} />
      </div>
    </>
  );
};

...
function MyApp() {
  ...
  const myOTPResponse = useSelector((state) => state.portalReducer.OTPVerify);
  const sentOTP = useSelector((state) => state.portalReducer.otp);
  ...
  const nextStep = (variant) => {
    if (myEmailResponse) {
      ...
      enqueueSnackbar("Enter your email address and submit before continue", {
        variant: "error",
      });
    }
  };
  ...
  const nextStep2 = (variant) => {
    if (myOTPResponse) {
      ...
      enqueueSnackbar(
        "Enter the correct OTP sent to your email address and then verify before continue",
        { variant: "error" }
      );
    }
  };
      ...
      >
        <Stepper.Step label="First step" description="Email Verification">
          <div className="custom-step-class position-relative">
            ...
            <Group position="center" mt="xl">
              <Button
                onClick={nextStep}
                className="email-verification--next-btn"
        ...
        <Stepper.Step label="Second step" description="OTP Verification">
          ...
          <Group
            position="center"
            mt="xl"
            style={{ position: "relative", top: "-75px", left: "" }}
          >
        ...
        <Stepper.Step label="Final step" description="Change Password">
          <div className="custom-step-class position-relative">
            ...
            <Button
              ...
              style={{ position: "relative", top: "-62px", left: "39%" }}
            >
              Back
            </Button>
          </div>
